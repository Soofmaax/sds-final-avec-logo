...
/**
 * Sanitise les données d'entrée en nettoyant les chaînes de caractères
 * @param data - Objet contenant les données à sanitiser
 * @returns Objet avec les données sanitisées
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const contact = await connectWithRetry(async () => {
      return prisma.contact.create({
        data: {
          ...
          projectType: sanitizedData.projectType as any,
          budget: sanitizedData.budget ? parseInt(sanitizedData.budget as string) : null,
          timeline: sanitizedData.timeline as string | undefined,
          source: (sanitizedData.source as string) || 'contact_form',
          status: 'NEW',
          ...metadata,
        },
      });
    ...
    try {
      ...
      await sendContactEmail({
        to: sanitizedData.email as string,
